// Разовая миграция: локальная встроенная MongoDB (D:\wyre\data\mongodb) → Atlas.
// Поднимает mongod на 127.0.0.1:27018, копирует все коллекции базы wyre
// с сохранением _id, затем аккуратно гасит mongod.
...
try {
  ...
  let atlas;
  try {
    atlas = await connectWithRetry(ATLAS_URI, 'Atlas (проверьте Network Access 0.0.0.0/0 и пользователя)', 10);
  } catch (error) {
    throw new Error('Atlas недоступен: ' + error.message);
  }
  console.log('Atlas подключён.');

  const source = local.db('wyre');
  const target = atlas.db('wyre');
  const names = await source.listCollections().toArray();
  let totalDocs = 0;
  for (const { name } of names) {
    const docs = await source.collection(name).find({}).toArray();
    if (!docs.length) {
      console.log(`  ${name}: пусто, пропускаем`);
      continue;
    }
    const result = await target.collection(name).insertMany(docs, { ordered: false });
    totalDocs += result.insertedCount ?? docs.length;
    console.log(`  ${name}: ${docs.length} документов (вставлено ${result.insertedCount ?? docs.length})`);
  }
  console.log(`Итого перенесено документов: ${totalDocs}, коллекций: ${names.length}`);

  await atlas.close();
  await local.close();
  await fetch('http://127.0.0.1:27018', { method: 'GET' }).catch(() => undefined);
  try {
  ...
  mongod.kill();
  console.log('Локальный mongod остановлен. Миграция завершена.');
} catch (error) {
  mongod.kill();
